...
<Props of="FieldSelectMulti" />

The `<FieldSelectMulti />` component is composed of an `<SelectMulti />` component, `<Label />` component, and a validation message.

By default, the label will render directly above the input field. Use the `inline` prop to render the label and select box on a single line.
...
### FieldSelectMulti Validation

If you wish to indicate validation errors, `<FieldSelectMulti />` accepts the `validationMessage` prop.
...
Note: In the current implementation, any `ValidationMessage` set in a `<FieldSelectMulti />` component will be overridden by `ValidationMessages` set in the parent `<Form />` component. Because of this, it is highly recommended that you only ever directly set `ValidationMessage` if a `<FieldSelectMulti />` is outside a `<Form />`.
